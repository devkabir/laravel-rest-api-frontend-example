<template>
  ...
  <div class="p-4  bg-white rounded shadow space-y-6">
    ...
    <TextInput label="Title" type="text" v-model="taskForm.name" :error="formError.name?.[0]" />
    <TextAreaInput label="Description" v-model="taskForm.description" :error="formError.description?.[0]" />
    ...
    <p v-if="formError.selectedUsers?.length" class="danger">{{ formError.selectedUsers[0] }}</p>
...
const { error, loading, formError } = storeToRefs(useFetchStore())
